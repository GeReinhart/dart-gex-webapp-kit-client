...
class User {
  ...
  @override
  String toString() => "User: userId:${_userId}, email:${_email}, displayName:${_displayName}, givenName:${_givenName}, familyName:${_familyName}, imageUrl:${_imageUrl}";
  
  User clone() {
    return new User(userId:_userId, email:_email,displayName:_displayName,familyName:_familyName,givenName:_givenName, imageUrl:_imageUrl);
  }
}
...
class GoogleAuthenticator extends Authenticator {
  ...
  @override
  void login(){
    _auth.login(immediate: true, onlyLoadToken: false)
      .then(_oauthReady)
      .catchError((e) {
      ...
      return HttpRequest.request(url, requestHeaders: headers)
        .then((HttpRequest request) {
          if (request.status == 200) {
            ...
            User user = new User(userId: token.userId, email: token.email, displayName: displayName,givenName:givenName ,familyName:familyName ,imageUrl: imageUrl) ;
